...
// Initialize Google Translate (you'll need to set GOOGLE_TRANSLATE_API_KEY in your environment)
let translate;
...
exports.translateLanguageWords = async (req, res) => {
    try {
        const { language_id, target_language_code } = req.body;
        
        if (!translate) {
            return res.json({
                status: 'error',
                msg: 'Translation service not configured. Please set GOOGLE_TRANSLATE_API_KEY environment variable.'
            });
        }

        // Get the target language
        const targetLanguage = await Language.findById(language_id);
        if (!targetLanguage) {
            return res.json({
                status: 'error',
                msg: 'Target language not found'
            });
        }

        // Find English language (assuming code 'en')
        const englishLanguage = await Language.findOne({ code: 'en' });
        if (!englishLanguage) {
            return res.json({
                status: 'error',
                msg: 'English language not found. Please create English language with code "en" first.'
            });
        }

        // Get all English language words
        const englishWords = await LanguageWord.find({ language_id: englishLanguage._id });
        if (englishWords.length === 0) {
            return res.json({
                status: 'error',
                msg: 'No English words found to translate'
            });
        }

        // Get all words to get the word names
        const words = await Word.find();
        const wordsMap = {};
        words.forEach(word => {
            wordsMap[word._id] = word.name;
        });

        let translatedCount = 0;
        let errorCount = 0;

        // Translate each English word
        for (const englishWord of englishWords) {
            try {
                const [translation] = await translate.translate(englishWord.value, target_language_code);
                
                // Check if translation already exists for this word in target language
                const existingTranslation = await LanguageWord.findOne({
                    language_id: language_id,
                    word_id: englishWord.word_id
                });

                if (existingTranslation) {
                    // Update existing translation
                    existingTranslation.value = translation;
                    await existingTranslation.save();
                } else {
                    // Create new translation
                    await LanguageWord.create({
                        language_id: language_id,
                        word_id: englishWord.word_id,
                        value: translation
                    });
                ...
                console.error(`Translation error for word ${wordsMap[englishWord.word_id]}:`, error);
                errorCount++;
            }
        }

        // Refresh settings after translation
        getSettings();

        res.json({
            status: 'success',
            msg: `Translation completed. ${translatedCount} words translated successfully.${errorCount > 0 ? ` ${errorCount} words failed.` : ''}`,
        ...
        res.json({
            status: 'error',
            msg: 'Translation failed: ' + error.message
        });
    }
};
